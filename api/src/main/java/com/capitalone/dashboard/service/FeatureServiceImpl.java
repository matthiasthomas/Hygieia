...
public class FeatureServiceImpl implements FeatureService {
	...
	public DataResponse<List<Feature>> getStory(ObjectId componentId,
		...
		if ((component == null)
				...
				|| CollectionUtils.isEmpty(component.getCollectorItems().get(
						CollectorType.AgileTool))
				|| (component.getCollectorItems().get(CollectorType.AgileTool)
						.get(0) == null)) {
	...
	public DataResponse<List<Feature>> getRelevantStories(ObjectId componentId,
		...
		List<Feature> relevantStories = getFeaturesForCurrentSprints(teamId,
				projectId, item.getCollectorId(),
				agileType.isPresent() ? agileType.get() : null, false,(String)item.getOptions().get("featureTool"));
	...
	@Override
	public DataResponse<List<Feature>> getFeatureEpicEstimates(
			ObjectId componentId, String teamId, String projectId,
		...
		List<Feature> relevantFeatureEstimates = getFeaturesForCurrentSprints(
				teamId, projectId, item.getCollectorId(),
				agileType.isPresent() ? agileType.get() : null, true,(String)item.getOptions().get("featureTool"));
		...
		for (Feature tempRs : relevantFeatureEstimates) {
			...
			// if estimateMetricType is hours accumulate time estimate in
			// minutes for better precision ... divide by 60 later
			double estimate = getEstimate(tempRs, estimateMetricType);

			feature.setsEstimate(String.valueOf(Double.valueOf(feature
					.getsEstimate()) + estimate));
		}
		...
		if (isEstimateTime(estimateMetricType)) {
			...
			for (Feature f : epicIDToEpicFeatureMap.values()) {
				f.setsEstimate(String.valueOf(Double.valueOf(f.getsEstimate()) / 60));
			}
		}
		...
		return new DataResponse<>(new ArrayList<>(
				epicIDToEpicFeatureMap.values()), collector.getLastExecuted());
	}

	@SuppressWarnings({"CPD-END"})
	@Override
	public DataResponse<SprintEstimate> getAggregatedSprintEstimates(
			ObjectId componentId, String teamId, String projectId,
			Optional<String> agileType, Optional<String> estimateMetricType) {
		Component component = componentRepository.findOne(componentId);
		if ((component == null)
		...
		SprintEstimate estimate = getSprintEstimates(teamId, projectId,
				item.getCollectorId(), agileType, estimateMetricType,(String)item.getOptions().get("featureTool"));
	...
	public DataResponse<List<Feature>> getTotalEstimate(ObjectId componentId,
		...
		SprintEstimate estimate = getSprintEstimates(teamId,
				FeatureCollectorConstants.PROJECT_ID_ANY,
				item.getCollectorId(), agileType, estimateMetricType,(String)item.getOptions().get("featureTool"));

		List<Feature> list = Collections.singletonList(new Feature());
		list.get(0).setsEstimate(formatter.format(estimate.getTotalEstimate()));
	...
	public DataResponse<List<Feature>> getInProgressEstimate(
			ObjectId componentId, String teamId, Optional<String> agileType,
			Optional<String> estimateMetricType) {
		Component component = componentRepository.findOne(componentId);
		...
		List<Feature> list = Collections.singletonList(new Feature());
		list.get(0).setsEstimate(
				formatter.format(estimate.getInProgressEstimate()));
	...
	public DataResponse<List<Feature>> getDoneEstimate(ObjectId componentId,
		...
		List<Feature> list = Collections.singletonList(new Feature());
		list.get(0).setsEstimate(
				formatter.format(estimate.getCompleteEstimate()));
	...
	private SprintEstimate getSprintEstimates(String teamId, String projectId,
			ObjectId collectorId, Optional<String> agileType,
			Optional<String> estimateMetricType,String featureTool) {
		List<Feature> storyEstimates = getFeaturesForCurrentSprints(teamId,
				projectId, collectorId, agileType.isPresent() ? agileType.get()
						: null, true, featureTool);

		double totalEstimate = 0;
		double wipEstimate = 0;
		double doneEstimate = 0;

		for (Feature tempRs : storyEstimates) {
			String tempStatus = tempRs.getsStatus() != null ? tempRs
					.getsStatus().toLowerCase() : null;
		...
		double openEstimate = totalEstimate - wipEstimate - doneEstimate;
	 ...
	 * @param minimal
	 *            if the resulting list of Features should be minimally
	 *            populated (see queries for fields)
	 * @return
	 */
	private List<Feature> getFeaturesForCurrentSprints(String teamId,
			String projectId, ObjectId collectorId, String agileType,
			boolean minimal,String featureTool) {
		...
		if (FeatureCollectorConstants.SPRINT_KANBAN.equalsIgnoreCase(agileType)) {
			/*
			 * A feature is part of a kanban sprint if any of the following are
			 * true: - the feature does not have a sprint set - the feature has
			 * a sprint set that does not have an end date - the feature has a
			 * sprint set that has an end date >= EOT
			 * (9999-12-31T59:59:59.999999)
			 */
			rt.addAll(featureRepository.findByNullSprints(teamId, projectId,
					collectorId, minimal));
			rt.addAll(featureRepository.findByUnendingSprints(teamId,
					projectId, collectorId, minimal));
		} else {
			// default to scrum
			/*
			 * A feature is part of a scrum sprint if any of the following are
			 * true: - the feature has a sprint set that has start <= now <= end
			 * and end < EOT (9999-12-31T59:59:59.999999)
			 */ 
			rt.addAll(featureRepository.findByActiveEndingSprints(teamId,
					projectId, collectorId, now, minimal,featureTool));
		}
	...
	private DataResponse<List<ProjectBoard>> getEmptyBoardDataResponse() {
		ProjectBoard f = new ProjectBoard();
		List<ProjectBoard> l = new ArrayList<>();
		l.add(f);
		return new DataResponse<>(l, 0);
	}
	...
	private boolean isEstimateTime(Optional<String> estimateMetricType) {
		return estimateMetricType.isPresent()
				&& FeatureCollectorConstants.STORY_HOURS_ESTIMATE
						.equalsIgnoreCase(estimateMetricType.get());
	}

	private boolean isEstimateCount(Optional<String> estimateMetricType) {
		return estimateMetricType.isPresent()
				&& FeatureCollectorConstants.STORY_COUNT_ESTIMATE
						.equalsIgnoreCase(estimateMetricType.get());
	}

	private double getEstimate(Feature feature, Optional<String> estimateMetricType) {
		double rt = 0;
		...
		if (isEstimateTime(estimateMetricType)) {
			...
			// default to story points since that should be the most common use
			// case
			if (!StringUtils.isEmpty(feature.getsEstimate())) {
				try {
					rt = Double.parseDouble(feature.getsEstimate());
				} catch (NumberFormatException nfe) {
					rt = 0;
					LOG.error(
							"Could not parse estimate for '"
									+ feature.getsName() + "', number '"
									+ feature.getsNumber()
									+ "', have estimate: "
									+ feature.getsEstimate(), nfe);
					ApplicationDBLogger.log(
							HygieiaConstants.API,
							"FeatureServiceImpl.getEstimate",
							"Could not parse estimate for '"
									+ feature.getsName() + "', number '"
									+ feature.getsNumber()
									+ "', have estimate: "
	...
	@Override
	public DataResponse<Map<String, VelocityData>> getBoardVelocityDetail(
			String projectId) {
		Map<String, VelocityData> velocityData = new HashMap<String, VelocityData>();
		ProjectBoard board = new ProjectBoard();
		if (projectId != "") {
			board = boardRepository.findByProjectId(Long.valueOf(projectId));
			if (board != null) {
		...
		return new DataResponse<>(velocityData, 0);
	}
	...
	private void manipulateData(String month, long estimated, long completed,
		...
		if (map.containsKey(month)) {
			...
			map.put(month, new VelocityData(estimated, completed));
		}
	}

	public DataResponse<ProjectBoard> getProjectBoardVelocityDetail(
			String projectId, String boardId) {
		ProjectBoard board = new ProjectBoard();
		if (projectId != "") {
			board = boardRepository.findByProjectIdAndBoardId(
					Long.valueOf(projectId), Long.valueOf(boardId));
		}
		return new DataResponse<>(board, 0);
	}
}