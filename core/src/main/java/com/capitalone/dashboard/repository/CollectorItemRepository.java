...
public interface CollectorItemRepository extends BaseCollectorItemRepository<CollectorItem> {
    ...
    @Query(value="{'options.appname' : ?1, '_class' : ?0}")
    List<CollectorItem> findByCollectorClassAndAppname(String collectorClass, String appName);
    
    @Query(value="{'_class' : ?0}")
    List<CollectorItem> findByCollectorClass(String collectorClass);
}
